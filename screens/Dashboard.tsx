    ...
    <div className="pb-16 md:pb-24 bg-white min-h-screen text-slate-800 animate-fadeIn">
      ...
      <div className="px-4 md:px-6 space-y-4 md:space-y-6 stagger-container">
        ...
        {hasMatches ? (
            ...
            <div className="w-full aspect-[4/3] rounded-[32px] bg-slate-900 relative overflow-hidden flex flex-col items-center justify-center text-center p-6 border-2 border-dashed border-slate-700 group animate-fadeInUp">
                {isAdmin ? (
                    <button 
                        onClick={() => onNavigate('matchCreate')}
                        className="flex flex-col items-center gap-4 hover-scale transition-all btn-press"
                    >
                        <div className="w-20 h-20 rounded-full bg-primary/20 flex items-center justify-center border-2 border-primary animate-pulse animate-glow">
