...
interface ReservationManagementProps {
  ...
  onMarkReservationPaid: (reservationId: string) => void;
    ...
    <div className="pb-8 bg-secondary min-h-screen">
      ...
      <div className="sticky top-0 z-50 bg-gradient-to-br from-slate-900 to-slate-800 px-4 pt-6 pb-4 border-b border-white/10">
        ...
        <div className="relative mb-3">
          ...
          <input type="text" placeholder="Takım veya kişi ara..." value={searchTerm}
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          <FilterTab label="Tümü"       count={stats.all}       active={filter === 'all'}       onClick={() => setFilter('all')} />
          <FilterTab label="Bekliyor"   count={stats.pending}   active={filter === 'pending'}   onClick={() => setFilter('pending')}   color="yellow" />
          <FilterTab label="Onaylı"     count={stats.confirmed} active={filter === 'confirmed'} onClick={() => setFilter('confirmed')} color="green" />
          <FilterTab label="Tamamlandı" count={stats.completed} active={filter === 'completed'} onClick={() => setFilter('completed')} color="blue" />
          <FilterTab label="İptal"      count={stats.cancelled} active={filter === 'cancelled'} onClick={() => setFilter('cancelled')} color="red" />
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-surface w-full max-w-sm rounded-3xl border border-white/10 p-6 shadow-2xl animate-slide-up">
            ...
            <div className="flex gap-3">
              <button onClick={() => { setShowRejectModal(false); setSelectedReservation(null); setRejectReason(''); }}
...
const FilterTab: React.FC<{
  label: string; count: number; active: boolean; onClick: () => void; color?: 'yellow' | 'green' | 'blue' | 'red';
  ...
  return (
    <button onClick={onClick}
      className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-xs font-bold whitespace-nowrap transition-all ${
        active ? (color ? colors[color] : 'border-primary/30 bg-primary/10 text-primary') : 'border-white/10 bg-white/5 text-slate-400 hover:bg-white/10'
      }`}
    >
      <span>{label}</span>
      <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-black ${active ? 'bg-white/20' : 'bg-white/10'}`}>{count}</span>
    </button>
  );
};
...
const ReservationCard: React.FC<{
  ...
  const statusMap = {
    ...
    cancelled: { text: 'text-red-500',    icon: 'cancel' },
    completed: { text: 'text-blue-500',   icon: 'task_alt' },
  };
  const payMap = {
  ...
  const pay = payMap[r.paymentStatus];
  ...
  const formatDuration = (dk: number) =>
    dk < 60 ? `${dk} dk` : dk % 60 === 0 ? `${dk / 60} saat` : `${Math.floor(dk / 60)} saat ${dk % 60} dk`;

  const payMethodLabel = { credit_card: 'Kart', bank_transfer: 'Havale', cash: 'Nakit' };

  return (
    <div className="bg-surface rounded-2xl border border-white/5 p-4 hover:border-primary/20 transition-all">
      {/* Header row */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-0.5">
            <Icon name={st.icon} size={15} className={st.text} />
            <h4 className="font-bold text-white text-sm">{r.teamName}</h4>
          </div>
          <p className="text-[11px] text-slate-400">{r.venueName}</p>
        </div>
        <div className="text-right">
          <p className="text-lg font-black text-primary">{r.price.toLocaleString('tr-TR')}₺</p>
          <div className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${pay.bg} ${pay.text} mt-0.5`}>{pay.label}</div>
        </div>
      </div>

      {/* Date / Time row */}
      <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400 mb-3 pb-3 border-b border-white/5">
        <div className="flex items-center gap-1">
          <Icon name="calendar_today" size={13} />
          <span>{new Date(r.date + 'T12:00:00').toLocaleDateString('tr-TR', { day: 'numeric', month: 'long' })}</span>
        </div>
        <div className="flex items-center gap-1">
      ...
      {r.depositRequired && r.depositAmount ? (
        <div className={`flex items-center justify-between rounded-xl px-3 py-2 mb-3 ${
          r.depositPaidAt ? 'bg-green-500/8 border border-green-500/20' : 'bg-yellow-500/8 border border-yellow-500/20'
        }`}>
          <div className="flex items-center gap-1.5">
            <Icon name={r.depositPaidAt ? 'check_circle' : 'payments'} size={14} className={r.depositPaidAt ? 'text-green-400' : 'text-yellow-400'} />
            <span className="text-[11px] font-bold text-slate-300">Kapora: {r.depositAmount.toLocaleString('tr-TR')}₺</span>
          </div>
          <span className={`text-[10px] font-black ${r.depositPaidAt ? 'text-green-400' : 'text-yellow-500'}`}>
            {r.depositPaidAt ? 'Ödendi ✓' : r.paymentMethod ? payMethodLabel[r.paymentMethod] + ' Bekleniyor' : 'Bekleniyor'}
          </span>
        </div>
      ) : null}
      ...
      <div className="flex flex-wrap items-center gap-2 text-[11px] text-slate-400 mb-3">
        <div className="flex items-center gap-1"><Icon name="person" size={12} /><span>{r.contactPerson}</span></div>
        <span className="text-slate-700">·</span>
        <div className="flex items-center gap-1"><Icon name="phone" size={12} /><span>{r.contactPhone}</span></div>
      </div>

      {/* Hold expired warning */}
      {r.status === 'pending' && r.paymentStatus !== 'paid' && holdExpired && (
        <div className="flex items-center gap-2 bg-red-500/8 border border-red-500/20 rounded-xl px-3 py-2 mb-3">
          <Icon name="timer_off" size={13} className="text-red-400" />
          <span className="text-[11px] text-red-400 font-bold">Hold süresi doldu — otomatik iptal bekleniyor</span>
        </div>
      )}
      ...
      {r.status === 'cancelled' && r.cancelReason && (
        <div className="bg-red-500/8 border border-red-500/20 rounded-xl p-3 mb-3">
          <p className="text-xs text-red-400"><strong>İptal Nedeni:</strong> {r.cancelReason}</p>
        </div>
      )}
      ...
      {r.notes && (
        <div className="bg-white/4 rounded-xl p-3 mb-3">
          <p className="text-[11px] text-slate-300">{r.notes}</p>
        </div>
      )}
      ...
      {/* Timestamps footer */}
      <div className="flex items-center gap-2 text-[10px] text-slate-600 mt-3 flex-wrap">
        <span>Oluşturuldu: {new Date(r.createdAt).toLocaleString('tr-TR', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
        {r.confirmedAt && <span>· Onaylandı: {new Date(r.confirmedAt).toLocaleString('tr-TR', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>}
      </div>
    </div>
  );
};
